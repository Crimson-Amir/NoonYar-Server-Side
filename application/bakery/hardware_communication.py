...
async def current_cook_customer(
    ...
    if urgent_id and time_per_bread and state_active and state_customer_id:
        ...
        if ticket_id_raw and int(ticket_id_raw) == int(state_customer_id):
            ...
            if any(int(x) > 0 for x in original_counts[: len(bread_ids_sorted)]):
                ...
                return {
                    "customer_id": tid,
                    "original_breads": _base_breads_by_name(tid),
                    "urgent_breads": _get_grouped_urgent_breads_for_tickets(bakery_id, [int(tid)], bread_names).get(int(tid), {}),
                    "next_customer": False,
                    "urgent": True,
                    "urgent_id": urgent_id,
                }
    ...
    if can_show_urgent:
        ...
        if any(int(x) > 0 for x in original_counts[: len(bread_ids_sorted)]):
            ...
            return {
                "customer_id": tid,
                "original_breads": _base_breads_by_name(tid) if tid > 0 else {},
                "urgent_breads": _get_grouped_urgent_breads_for_tickets(bakery_id, [int(tid)], bread_names).get(int(tid), {}) if tid > 0 else {str(urgent_id): {"breads": _counts_to_name_map(original_counts)}},
                "next_customer": False,
                "urgent": True,
                "urgent_id": urgent_id,
            }
